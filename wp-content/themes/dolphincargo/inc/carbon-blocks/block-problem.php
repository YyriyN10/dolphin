<?php
	...
	add_action( 'carbon_fields_register_fields', 'dolphincargo_block_problem' );

	function dolphincargo_block_problem(){
		Block::make( __( 'Problem' ) )
		     ->add_fields( array(
			     Field::make_text('dolphincargo_block_problem_title', 'Заголовок блоку'),
			     Field::make_complex('dolphincargo_block_problem_list', 'Перелік проблем')
			          ->add_fields(array(
				          Field::make_text('name', 'Назва проблеми'),
				          Field::make_complex('reason_list', 'Перелік підстав через яку утворюється')
				            ->add_fields(array(
					            Field::make_rich_text('text', 'Текст підстави')
				            ))
			          )),
		     ) )
		     ...
		     ->set_render_callback( function ( $fields, $attributes, $inner_blocks ) {
			     ...
			     <?php if( !empty( $fields['dolphincargo_block_problem_title'] ) && !empty( $fields['dolphincargo_block_problem_list']) ):?>

				     <section class="block-problem indent-top-big indent-bottom-small animation-tracking" >
					     <?php get_template_part('template-parts/decor-lines');?>

					     <div class="light"><img src="<?php echo THEME_PATH;?>/assets/img/light-h-3.png" alt="<?php echo the_title();?>"></div>
                 ...
                 <svg class="bg-pic" width="890" height="938" viewBox="0 0 890 938" fill="none" xmlns="http://www.w3.org/2000/svg">
                   <g style="mix-blend-mode:luminosity" opacity="0.2">
                     ...
                     <path d="M213.797 495.391C251.78 392.787 352.357 331.376 456.237 338.413C456.793 338.157 457.147 337.826 457.501 337.495C466.506 332.512 483.41 318.671 494.123 309.7C366.984 263.788 226.261 328.819 179.176 456.007C141.945 556.579 174.548 665.677 252.123 730.665C200.768 666.604 183.259 577.88 213.797 495.391Z" fill="#464646"/>
						     ...
						     <div class="row">
							     <h2 class="block-title big-title col-xl-7 col-lg-9 offset-xl-5 offset-lg-3 first-up"><?php echo $fields['dolphincargo_block_problem_title'];?></h2>
							     <ul class="problem-list col-12 second-up">
								     <?php foreach( $fields['dolphincargo_block_problem_list'] as $index=>$item ):?>
									     <li class="problem">
										     <p class="number">
											     <?php if( ( $index + 1 ) < 10):?>
												     0<?php echo $index + 1;?>
										     ...
										     <?php if( !empty( $item['reason_list'] ) ):?>
											     <ul class="reason-list">
												     <?php foreach( $item['reason_list'] as $reason ):?>
													     <li><?php echo wpautop( $reason['text'] );?></li>
												     <?php endforeach;?>
											     </ul>
										     <?php endif;?>
									     </li>
